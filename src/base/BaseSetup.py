...
class BaseSetup:
    ...
    def solve_instance(self, instance):
        ...
        if self.solver.solve_puzzle():
            print(f"{self.game_type.capitalize()} puzzle #{instance['number']} solved in {self.solver.computing_time}s")
            solution = self.solver.get_model()
            instance['solution'] = solution
            self.save_json(instance, f"{self.save_folder}/instance_{instance['number']}")
        else:
            print(f"{self.game_type.capitalize()} puzzle #{instance['number']} has no solution")
